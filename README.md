...
For Boltz-2, enable the optional low-memory mode with:

```bash
boltz predict input.yaml --low_memory
```

It automatically uses BF16 on supported CUDA GPUs, FP16 otherwise, and FP32 on
CPU. As a rough guide from the checked BF16 examples, peak RAM usage drops by
about **7–22%**, and peak allocated GPU memory by **up to 9%**; small inputs may
see little GPU memory reduction. Savings depend on the input and output options.
Sample counts, inference steps and MSA contents are preserved, but inference can
be slower and predictions can change. See the [memory options](docs/prediction.md#reducing-boltz-2-inference-memory)
for precision and output controls.
